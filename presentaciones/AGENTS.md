...
* **Ubicación de Hojas de Estilo (style.css):** Slidev solo busca el archivo `style.css` en la misma carpeta que contiene el archivo markdown principal de compilación. Si modularizas una nueva presentación en una subcarpeta (ej. `subcarpeta/principal.md`), debes copiar `style.css` a esa subcarpeta (`subcarpeta/style.css`), de lo contrario Slidev cargará el tema por defecto blanco e ignorará los estilos del proyecto.
